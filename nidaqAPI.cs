...
namespace Cuteffi_rebuild
{
    class nidaqAPI
    {
        ...
        public double[] maxrms = new double[2] { 0, 0 };
        public double[] allrms0 = new double[1280];//
        ...
        public double[] vecSP = new double[13];
        ...
        private void AnalogInCallback(IAsyncResult ar)
        {
            if (runningTask != null && runningTask == ar.AsyncState)
            {
                ...
                //
                if (SplashScreen.cut.panel11.Visible==true && SplashScreen.cut.panel1.Visible == false  )
                {
                    //執行優化
                    //小助手演算法
                    double varSP;
        ...
        //public static double SPOptimization(double maxSP)
        {
            ...
            //for (int i = 1; i <= 12; i++)
            //{
            //    int varC = Array.IndexOf(arrayB, arrayB.Min());
            //    OptimizedSPlist[i - 1] = vecSP[varC + 2];
            //    OptimizedGlist[i - 1] = arrayB[varC];
            //    arrayB[varC] = 1000000000;
            //}
            //return OptimizedSP;
            double[] Alist = CUTeffiForm.Alist;
            double[] Glist = CUTeffiForm.Glist;
            //double[] G = CUTeffiForm.G;
